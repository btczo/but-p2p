# Messages
The but protocol specifies a set of [messages](https://en.bitcoin.it/wiki/Protocol_specification) that can be sent from peer to peer. `but-p2p` provides support for some of these messages.
...
As well as sending "tx" and "block" messages with ButK-js instances:
...
## List of Messages
### Version
The version message (`ver`) is used on connection creation, to advertise the type of node. The remote node will respond with its version, and no communication is possible until both peers have exchanged their versions. By default, but-js advertises itself as named `but-js` with the current version of the `but-p2p` package.
